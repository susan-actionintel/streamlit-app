...
usda_data_folder = 'usda-data/'
aws_access_key_id = st.secrets['access_key']
aws_secret_access_key = st.secrets['secret_key']
user_config_bucket = 'streamlit-config'
user_config_key = 'yaml.config'
...
response_config = s3.get_object(Bucket=user_config_bucket, Key=user_config_key)
config = yaml.safe_load(response_config["Body"])
response_image = s3.get_object(Bucket=bucket_name, Key=static_data_folder+logo_file)
...
# Wrap available content into login 
authenticator.login('Login', 'sidebar')
if st.session_state["authentication_status"]:
    # st.write(f'Welcome **{st.session_state["name"]}**!')
    # authenticator.logout('Logout', 'main', key='unique_key_river')
    with st.spinner('Loading ...'):
        river_tab, transit_tab, eta_tab, unloads_tab, exports_tab, user_tab = st.tabs(["MS river levels", "Lower MS tow transit", "Lower MS transit ETAs", "Grain barge lockings and unloads", "NOLA grain exports","Account configuration"])
        with river_tab:
        ...
        for data_type in usda_data_types:
            usda_data[data_type] = dataframe_from_s3_csv(bfpm_bucket_name,usda_data_folder+('usda-'+data_type+'.csv'))
    ...
    with user_tab:
        ...
        authenticator.logout('Logout', 'main', key='unique_key_river')
        try:
            if authenticator.reset_password(st.session_state["username"], 'Reset your password'):
                st.success('Password modified successfully')
        except Exception as e:
            st.error(e)
        try:
            if authenticator.update_user_details(st.session_state["username"], 'Update your user details'):
    ...
    if forgot_password:
        try:
            username_of_forgotten_password, email_of_forgotten_password, new_random_password = authenticator.forgot_password('Please enter your user name to reset password')
            if username_of_forgotten_password:
                st.success('Your new password will be sent via email.')
                # Random password should be transferred to user securely
            elif username_of_forgotten_password is False:
                st.error('Username not found')
        except Exception as e:
            st.error(e)
    forgot_username = st.checkbox('Forgot Username?')
    if forgot_username:
        try:
            username_of_forgotten_username, email_of_forgotten_username = authenticator.forgot_username('Please enter your email to get your username')
            if username_of_forgotten_username:
                st.success('Your username is on its way via email!')
    ...
    if new_user:
        try:
            if authenticator.register_user('Register here', preauthorization=False):
    ...
    st.sidebar.image(resize_logo)
    try:
        if authenticator.register_user('First time user?', preauthorization=False):
            st.success('User registered successfully')
    except Exception as e:
        st.error(e)
# %% Update user config file with any changes
with open('yaml.config', 'w') as file:
