...
file_date = '2023-08-19' # also used as end date throughout the code
...
input_filename = 'RawAISDashboardData_'+file_date+'.csv'
output_filename = 'tow_key_locs_data'+file_date+'.csv'
...
owner_type_dict = {'dry bulk':['ACBL','ARTCO','INGRAM','MARQUETTE','SCF','WESTERN RIVERS'],\
                   'liquids':['BLESSEY','ENTERPRISE MARINE','GENESIS MARINE','GOLDING','KIRBY','LEBEOUF BROS.'\
...
normal_stddev = 1.5 # standard deviations to consider "normal"
# %% Define functions
...
tows_tab, river_tab, exports_tab = st.tabs(["Lower MS transit", "MS river levels", "NOLA grain exports"])
with tows_tab:
    left_col, right_col = st.columns([0.3, 0.7],gap = 'medium')
    with left_col:
        st.image(resize_logo)
        st.subheader('User selections')
        owner_type = st.selectbox('Select commodity type',['dry bulk','liquids', 'all'])
        st.divider()
        tow_hp_lower_lim = st.slider('Select minimum tow horsepower',value = 6500,min_value = 2000, max_value = 10000, step = 100)
# %% Create an end of week date series
start_date = pd.to_datetime(start_date)
frequency = 'W'  # 'W' stands for weekly
num_weeks = np.floor((pd.to_datetime(file_date) - start_date).days/ 7)
# Create the series of dates
date_series = pd.date_range(start=start_date, periods=num_weeks, freq=frequency)
...
river_data_bench_mean, river_data_bench_std = weekly_benchmark_calc(river_data,pd.to_datetime(file_date),river_stage_bench_years)
...
tow_benchmark = tow_bench_data[((tow_bench_data['date']>=pd.to_datetime(file_date)-pd.Timedelta(tow_bench_years*365,"d"))
                            &(tow_bench_data['date']<=pd.to_datetime(file_date)))].copy()
...
tow_benchmark = pd.get_dummies(tow_benchmark,columns=['direction'])
...
tow_benchmark = tow_benchmark.asfreq('D').reset_index().fillna(0)
tow_benchmark['week_of_year'] = tow_benchmark['date'].apply(lambda x: x.weekofyear)
tow_benchmark['year'] = tow_benchmark['date'].apply(lambda x: x.year)
tow_benchmark = tow_benchmark.drop(columns=['date']).groupby(by=['week_of_year','year']).sum().reset_index()
tow_benchmark = tow_benchmark.drop(columns=['year']).groupby(by=['week_of_year']).agg(['mean','std']).reset_index()
...
daily_series = pd.date_range(start=start_date, end=file_date, freq='d')
...
# %% Weekly tow analysis
tows_weekly = tows_daily.set_index('date').resample('W',convention='start').sum()
tows_weekly = tows_weekly[tows_weekly.index.isin(date_series)].reset_index()
tows_weekly['week_of_year'] = tows_weekly['date'].apply(lambda x: x.weekofyear)
tows_weekly['date'] = tows_weekly['date'].dt.date
tows_weekly = tows_weekly.merge(tow_benchmark[['week_of_year','downstream_bench_mean','downstream_bench_std']],
                              how = 'left',on = 'week_of_year',) #add weekly benchmark for downstream
tows_weekly = tows_weekly.merge(tow_benchmark[['week_of_year','upstream_bench_mean','upstream_bench_std']],\
                              how = 'left',on = 'week_of_year') #add weekly benchmark for downstream
...
fig_d.update_yaxes(
    ...
    zerolinewidth = 1,
    linecolor = 'silver',
    title_text = 'Tows moving toward NOLA'  
)
fig_d.update_xaxes(
    zerolinecolor='#4682b4',
    zerolinewidth = 1,
...
fig_r.update_layout(
    ...
    yaxis=dict(showgrid=False),
    font=dict(color='#c0c0c0'),
    title = (' MS River Stage (Vicksburg Proxy)')
)
fig_r.update_yaxes(
    zerolinecolor='#4682b4',
    zerolinewidth = 1,
...
with tows_tab:
    with right_col:
        st.title('Barge AI - Lower MS River Intel')
        ...
        st.write('The tow counts in the table below correspond to the charts above.')
        st.write(tows_weekly_display.round(0))
        st.download_button('Download data', tows_weekly_display.to_csv(index = False),file_name = 'tow_counts.csv') 
with river_tab:
